    ...
    internal static class DotNetConfigHelper
    {
        ...
        public static XDocument CreateNetFxAppConfig(IList<string> references)
        {
            ...
            var config = new XElement("System.Windows.Forms.ApplicationConfigurationSection",
                ...
                new XElement("add", new XAttribute("key", "EnableWindowsFormsHighDpiAutoResizing"), new XAttribute("value", "false"))
                );
            return new XDocument(
                new XElement("configuration", new XElement(startup), new XElement(config), new XElement(runtime)));
        }
    }
}
