...
namespace Huarui.STARLine
{
    class Util
    {
        ...
        public static ModuleErrors ParseResponse(string response)
        {
            int index = response.IndexOf("er99/00");
            if (index < 0)
                return null;
            ModuleErrors errors = new ModuleErrors();
            string[] slaveErrors = response.Substring(index + 8).Split(' ');
            foreach(string p in slaveErrors)
            {
                ModuleError e = new ModuleError();
                e.Module = p.Substring(0, 2);
                e.MainError = int.Parse(p.Substring(2, 2));
                e.SlaveError = int.Parse(p.Substring(5, 2));
                errors.Add(e);
            }
            return errors;
        }
